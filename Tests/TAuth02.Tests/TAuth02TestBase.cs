...
namespace TAuth02.Tests
{
    public abstract class TAuth02TestBase : AbpIntegratedTestBase<TAuth02TestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected TAuth02TestBase()
        {
        ...
        protected void UsingDbContext(Action<TAuth02DbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<TAuth02DbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<TAuth02DbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<TAuth02DbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<TAuth02DbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TAuth02DbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<TAuth02DbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TAuth02DbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<TAuth02DbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<TAuth02DbContext, Task<T>> func)
        {
            T result;
